...
<body>
		...
		<div class="main-content-inner">
			...
			<div class="page-content">
				<div class="page-header">
					<h2 class="tm-block-title d-inline-block">Thêm sách</h2>
				</div>
				<!-- /.page-header -->
				<div class="row">
					<div class="col-xs-12">
						...
						<form:form action="${createURL}" method="post" commandName="model"
							enctype="multipart/form-data">

							<form:hidden path="wareHouseId" class="listWW" value="" />
							...
							<div class="row">
								<div class="col-md-7">
									<div class="form-group">
										...
										<form:errors path="bookCode" style="color:red;"
											cssClass="error" />
									</div>
									<div class="form-group">
									...
									<div class="row">
										...
										<div class="form-group mb-3 col-xs-12 col-sm-6">
											...
											<form:input path="discount" type="text"
												class="form-control validate" />
									...
									<div class="form-group">
										...
										<form:textarea path="shortContent"
											class="form-control validate" rows="3" />
										<form:errors path="shortContent" style="color:red;"
											cssClass="error" />
									</div>

									<div class="form-group">
										<label for="description">Nội dung</label>
										<form:textarea path="context" class="form-control validate"
											rows="3" />
										<form:errors path="context" style="color:red;"
											cssClass="error" />
									</div>
									<div class="form-group">
										<div class="row">
											...
											<div class="col-md-6">
												...
												<form:select class="custom-select tm-select-accounts"
													path="publisherCode" id="publisherCode">
													<option value="" selected="selected">Chọn nhà xuất
														bản</option>
													<form:options items="${publisher}" />
												</form:select>
												<form:errors path="publisherCode" style="color:red;"
													cssClass="error" />
											</div>
									...
									<div class="form-group">
										<div class="row">
											<div class="col-md-6">
												...
												<form:select class="custom-select tm-select-accounts"
													path="authorCode" id="authorCode">
													<option value="" selected="selected">Chọn tác giả</option>
													<form:options items="${author}" />
												</form:select>
												<form:errors path="authorCode" style="color:red;"
													cssClass="error" />
											</div>
										</div>
									</div>
									<div class="form-group">
										<div class="row">
											<form:select path="warehouseCode" class="selectWarehouse"
												style="width:100px;">
												<option value="" disabled selected>-- Chọn kho --</option>
												<form:options items="${warehouse}" />
											</form:select>

											<a style="margin-left: 20px;" type="button"
												class="btn btn-primary btn-sm " id="insertRow"> Xác nhận</a>
											<form:errors path="wareHouseId" style="color:red;"
												cssClass="error" />
											<div id="messagesModal" class="alert alert-danger"
												style="display: none;"></div>
											<table id="tableWareHouse" class="table table-bordered"
												width="100%">
												<thead>
													<tr>
								...
								<div class="col-md-3">
									<img id="review"
										src="<c:url value='/template/admin/image/NoImage.jpg'/>"
										width="90%" alt="">
									<div id="review"></div>
									<div class="form-group">
										...
										<form:input path="imageFile" type="file"
											class="form-control validate" />
										<form:errors path="imageFile" style="color:red;"
											cssClass="error" />

									</div>
									<div class="form-group ">
										<label for="category">Năm xuất bản</label>
										<form:input path="releaseYear" type="text"
											class="form-control validate" />
										<form:errors path="releaseYear" style="color:red;"
											cssClass="error" />
									</div>
									...
									<div class="form-group ">
										<label for="category">Gắn Tag</label>
										<form:select class="custom-select tm-select-accounts"
											path="tagCode" id="tagCode">
											<option value="" selected="selected">Chọn Tag</option>
											<form:options items="${tag}" />
										</form:select>
										<form:errors path="tagCode" style="color:red;"
											cssClass="error" />
									</div>
								</div>
							</div>
	...
	<script>
		...
		$("#insertRow").on("click",function(event) {
							...
							var selectedcode = $(
									".selectWarehouse option:selected").val();
							var selectedtext = $(
									".selectWarehouse option:selected").text();
							...
							if (arr.length != 0) {
								if (arr.includes(selectedcode) === true) {
									...
									cols += '<td name="tdCodeWareHouse">'
											+ selectedcode + '</td>';
									cols += '<td name="tdNameWareHouse">'
											+ selectedtext + '</td>';
									cols += '<td><input autocomplete="off" class="form-control validate aprroved-table" onkeypress="SetDefault();" onpaste="SetDefault();" oninput="SetDefault();" type="number" name="handle" ></td>';
								...
								if (selectedcode === "") {
									...
									cols += '<td name="tdNameWareHouse">'
											+ selectedtext + '</td>';
									cols += '<td><input autocomplete="off" id="mapw.value" class="form-control validate aprroved-table" onkeypress="SetDefault();" onpaste="SetDefault();" oninput="SetDefault();" type="number" name="mapw.value" ></td>';
									cols += '<td><button class="btn btn-danger rounded-0" id ="deleteRow"><i class="fa fa-trash"></i></button</td>';
